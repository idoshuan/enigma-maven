...
public class XMLLoader implements Loader {
    ...
    public Inventory load(String path) {
        ...
        return validateAndParse(enigma);
    }

    @Override
    public Inventory load(InputStream inputStream) {
        BTEEnigma enigma = parseXml(inputStream);
        return validateAndParse(enigma);
    }

    private Inventory validateAndParse(BTEEnigma enigma) {
    ...
    private BTEEnigma parseXml(InputStream inputStream) {
        try {
            JAXBContext context = JAXBContext.newInstance(BTEEnigma.class);
            Unmarshaller unmarshaller = context.createUnmarshaller();
            return (BTEEnigma) unmarshaller.unmarshal(inputStream);
        } catch (JAXBException e) {
            throw new ParseException("uploaded-file", "XML", e);
        }
    }
}
